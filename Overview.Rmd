...
yearDiffChart <-  ggplot(prePostAward, aes(yearDiff, n, text=paste('Difference:', yearDiff, "years, n=", n))) +
        geom_col(fill="#2d4059") +
  custom_theme +
...
yearDiffPrePostChartFilter <-  ggplot(prePostAwardPrePostWP, aes(yearDiff, n, text=paste('Difference:', yearDiff, "years, n=",n))) +
        geom_col(fill="#2d4059") +
  custom_theme +
...
# yearDiffPrePostChartFilter <-  ggplot(prePostAwardPrePostWPFilter, aes(yearDiff, n, text=paste('Difference:', yearDiff, "years, n=",n))) +
#         geom_col(fill="#2d4059") +
#   custom_theme
...
ggplotly(yearDiffPrePostChartFilter, tooltip="text") %>% 
  layout(font=t, 
         annotations = 
...
prePostAwardSumExcOldLevels <- wikipedia %>% 
        ...
        add_tally(name="total") %>% 
  mutate(beforeAfter = case_when(timeDiff >=0 ~ "on/after Hons",
                                 timeDiff<0 ~ "before Hons")) %>%
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
cumPageChartYearDiffYOY <- ggplot(cumPageYearDiff, aes(wikipediaYear, cumpagesDiffPercentYOY, text=paste(lag(wikipediaYear),"v", wikipediaYear,":",percent (cumpagesDiffPercentYOY)))) +
        geom_col(fill="#2d4059")+
        custom_theme+
