...
##fxing up naming issues and flattening ultiple honours to one record where earliest honour date is used as honours_date and highest level is honour level
...
Honours are issued with the following divisions:

1. General (G): awarded to Australian Citizens 
2. Military (M): warded to Australian Military
3. Honourary: General (HG) and Military (HM) honours can also be issued to non-citizens as honourary honours. A new distinction was created recently to honour permanent-residents. These used to be indicated as "HG", but are now indicated by "HP".

People can receive multiple Order of Australia honours in their lifetime. People can have their honours terminated by the Council, or return or refuse their honours.  Some honours are issued or recorded as anonymous on the honours Database.
...
<!-- #### 1. Anonymous Honours - <b>TO UPDATE</b> -->
...
<!-- Records from the PMO dataset that are listed as anonymous or teminated honours are imcomplete. Anonymous honours have only been if there has been an honour date, and a wikipedia page and a page creation date. If the record is missing the Honours date, they have been excluded from the dataset.  -->
...
table_nums(name = "angus1", "All Orders of Australia Receieved by Angus Houston")
```
...
The table below illustrates his record extracted from the data set based on individual recipients. It shows that his honour level refers to his most recent honour in 2015 and his first honour date refers to the first honour he received in 1990. 
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
table_nums(name = "angus2",  "Angus Houston's single record ")
```
<div style="<div style="margin-top:30px;">
</div>
`r table_nums("angus2")`
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
table_nums(name = "division",  "Breakdown of Divisions")
```
<div style="<div style="margin-top:30px;">
</div>
`r table_nums("division")`
...
The Australian Honors system began in 1975, and wikipedia was established on 15 January 2001. In order to calclate the time difference bewteen the honour date and the wikipedia creation date, any honour issued before 15 Jan 2001, has a "start date" of 15 January 2001. The table below shows the recipients first honour date (first_honours_date), the date the time difference is calculted on (new_honours_date), the date their wikipedia entry was created, and the difference in time from honour to page creation. 

<b>A.D Hope</b> received their honour in 1981. The earliest a wikpedia page could be created for them is 2001, and their page was created in 2004, so the difference is three years. Looking further down <b>Aaron Kierney</b> had a page created for them in 2012, and they received their honours in 2020. The time difference is -8 years, meaning their page was created 8 years <i>before</i> they received their honours. We can also see that number of days and weeks difference has been calculated. 
<br>
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
table_nums(name = "date_calcs",  "Date calculations (scroll to see full row)")

```
<div style="<div style="margin-top:30px;">
</div>
`r table_nums("date_calcs")`
...
<i>(Please note the charts in this section show all <b>recipients</b> of an Order of Australia. If they have received multiple honours, we are only including their first honour issued so it matches with one page created for them. The creation of the wikipedia page may not have occurred in the same year as the honour, but simply indicates if they have a page or not)</i>

<br>
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig1","Proportion of recipients who have wikipedia page")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig1")`
...
Over time the number of honours issued each year has increased. The line represents the average number of reward recipients who have a page, which is around 121 for each year.  In more recent years, there has been a higher number of recipients who have wikipedia pages, as the number of honours increases. 
<br>
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig2","Number of honours issued over time with or without a wikipedia page")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig2")`
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig3","Proportion of honours issued over time with or without a wikipedia page")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig3")`
...
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig4","Proportion of honours issued over time with or without a wikipedia page")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig4")`
...
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig5","Number of Honours levels awarded over time")
```
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig6","Proportion of Honours levels awarded over time")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig6")`
...
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig7","Proportion of honour recipients that have a wikipedia article page by highest honour received")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig7")`
...
pagePropAwardChart <- ggplot(pagePropAward, aes(award_comb, prop, fill=wikipedia_page,
        ...
        custom_theme+
        labs(title="Proportion of honour recipients that have a wikipedia article page by highest honour received", 
...
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig8","Number of honours awarded over time split by gender")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig8")`
...
While the number has been increasing, there is still a lower proportion of women who receive an honour. For the honours issued so far in 2020, 42% of the honours went to women.
<br>
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}

figure_nums(name = "fig9","Proportion of honours awarded over time split by gender")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig9")`
...
figure_nums(name = "fig10","Number of honour recipients that have a wikipedia article page by Gender")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig10")`
...
gender_diff_wp <- ggplot(gender_split_page, aes(honours_year, n, fill=wikipedia_page, text=paste(honours_year, gender, ":", n)))+
  ...
  custom_theme+
  scale_fill_manual(values=colours_wp)+
  labs(title="Number of honour recipients that have a wikipedia article page by Gender", subtitle = "Line represents total proportion to highlight differences",
...
figure_nums(name = "fig11","Honour Levels: Proportion of honour recipients that have a wikipedia article page by Gender")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig11")`
...
pagePropGenderAwardChart <- ggplot(pagePropGenderAward, aes(gender, prop, fill=wikipedia_page,text = paste(percent(prop)))) +
        geom_bar(position="fill", stat="identity") +
        custom_theme+
        labs(title="Proportion of honour recipients that have a wikipedia article page by Gender and Honours levels", 
             x="Order of Australia (membership levels)", y="Proportion of recipients") +
...
figure_nums(name = "fig12","Proportion of honour recipients that have a wikipedia article page by Gender and Honours level")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig12")`
...
pageAverageGenderAwardChart <- ggplot(pageAverageTotal, aes(gender, totalWPProp, fill=gender, text = paste(percent(totalWPProp)))) +
        ...
        custom_theme+
        labs(title="Proportion of honour recipients that have a wikipedia article page by Gender", subtitle = "Line represents total proportion to highlight differences",
             x="Order of Australia (membership levels)", y="Proportion of recipients") +
...
Whether an honour issued in a given year has a page or not tells us proportions of recipients that have a page, but it hides details on whether the wikipedia page was created for the recipient before or after they received their Order of Australia.

The below analysis looks at the difference in time between honour and page creation date. 

The first chart shows the amount of time by day, between receieving the honour and when their wikipedia page was created. There are a few different peaks shown, with the highest being twenty pages created on day zero, or the day that a person receives their award.  (The chart is quite pale as there are many frequency counts per day, but it's possible to brush over the chart with your mouse to highlight a particular area.) The average page creation each day is `r (round(award_pre_post_day_mean$mean, 2))` and on day zero it is 20.
<br>
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}

figure_nums(name = "fig13","Difference in time between honour and wikipedia page creation by day")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig13")`
...
award_pre_post_day_chart <-  ggplot(award_pre_post_day, aes(time_diff, total, text=paste('Difference:', time_diff, "days, n=", total))) +
        geom_col(fill="black") +
  custom_theme +
        labs(title="Difference in time between honour and wikipedia page creation by day", 
...
ggplotly(award_pre_post_day_chart, tooltip="text") %>% 
  layout(font=t, 
         title = list(text = paste0('Difference in time between honour and wikipedia page creation by day',
                                    '<br>',
                                    '<sup>',
...
figure_nums(name = "fig14","Difference in time between honour and wikipedia page creation by week")
```
<div style="<div style="margin-top:30px;">
</div>
`r figure_nums("fig14")`
...
award_pre_post_week_chart <-  ggplot(award_pre_post_week, aes(week_diff, total, text=paste('Difference:', week_diff, "weeks, n=", total))) +
        geom_col(fill="#02401B") +
  custom_theme +
        labs(title="Difference in time between honour and wikipedia page creation by week", 
...
ggplotly(award_pre_post_week_chart, tooltip="text") %>% 
  layout(font=t, 
         title = list(text = paste0('Difference in time between honour and wikipedia page creation',
                                    '<br>',
                                    '<sup>',
                                    'Negative numbers are those who have page created <i>before</i> honour, and postive have page created <i>after</i> honour',
                                    '</sup>')),
...
Rolling the time increments up to a year, shows us that while there is a peak of 209 pages created over the year the honour is received, there is a peak for all honour recipients to have a page created between 3 and seven years after receieving their honour. 

<br>
<br>
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
figure_nums(name = "fig15","Difference in time between honour and wikipedia page creation by year")
```
...
award_pre_post_chart <-  ggplot(award_pre_post, aes(year_diff, total, text=paste('Difference:', year_diff, "years, n=", total))) +
        geom_col(fill="#02401B") +
  custom_theme +
        labs(title="Difference in time between honour and wikipedia page creation", 
...
ggplotly(award_pre_post_chart, tooltip="text") %>% 
  layout(font=t, 
         title = list(text = paste0('Difference in time between honour and wikipedia page creation by year',
                                    '<br>',
                                    '<sup>',
                                    'Negative numbers are those who have page created <i>before</i> honour, and postive have page created <i>after</i> honour',
                                    '</sup>')),
...
```
This analysis though may be misleading, as we are including all honours issued since the start of the Order. We can exclude all those honours received before the start of wikipedia on Januray 15 2001 so we give all pages an "equal chance" of having a page created for them without any lag.
...
figure_nums(name = "fig16","Difference in time between honour and wikipedia page creation by year for all honours received after 15 Jan 2001")
```
...
ggplotly(award_pre_post_chart, tooltip="text") %>% 
  layout(font=t, 
         title = list(text = paste0('Difference in time between honour and wikipedia page creation (post wikipedia only)',
                                    '<br>',
                                    '<sup>',
                                    'Negative numbers are those who have page created <i>before</i> honour, and postive have page created <i>after</i> honour',
                                    '</sup>')),
...
week_diffAllYearChartLevel2 <- ggplot(week_diffAllYears2, aes(week_diff, award_comb, fill=n, text=paste("week", week_diff, ":", n)))+
  ...
  labs(title="Time of wikipedia  page creation for those who received honours after 15 Jan 2001", 
             x="weeks before or after honour page is created", y="Number of pages / honour type")


ggplotly(week_diffAllYearChartLevel2, tooltip="text") %>% 
  layout(font=t, 
         title = list(text = paste0('Weekly difference in time between honour and wikipedia page creation (post wikipedia only)',
                                    '<br>',
                                    '<sup>',
                                    'Showing 52 weeks prior and post honour week.',
                                    '</sup>')),
         annotations = 
           list(x = 1, y = -0.13, text = "All Recipients w. WP page receiving honour post-wikipedia 6mnths before or after n=223", 
                showarrow = F, xref='paper', yref='paper', 
                xanchor='right', yanchor='auto', xshift=1, yshift=-2,
                font=list(size=10)),
          autosize=F,
...
figure_nums(name = "fig19","Weekly difference in time  (52 weeks pre and post) between honour and wikipedia page creation (post wikipedia only)")
```
...
figure_nums(name = "fig20","Daily cumulative page creation of honours recipients")
```
...
figure_nums(name = "fig21","Annual wikipedia page creation for honours recipients")
```
...
figure_nums(name = "fig22","Annual number of honours awarded and wikipedia page creation")
```
...
layout(font=t, 
         annotations = 
           list(x = 1, y = -0.2, text = "All Recipients since 15 Jan 2001 n=24701", 
                showarrow = F, xref='paper', yref='paper', 
                xanchor='right', yanchor='auto', xshift=1, yshift=-2,
                font=list(size=10)),
          autosize=F,
...
figure_nums(name = "fig23","Difference in numbers: Annual number of honours awarded and wikipedia page creation")
```
...
Looking at page creation for all people who recievd an honour and have a wikipedia page, there were slightly more pages created for men than women before an honour was receieved.  Potentially the recogition of an order prompts page creation for women?
<br>
<br>

```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}

figure_nums(name = "fig24","Difference in pages created before or after hnours (all recipients with wikipedia page)")
```
...
figure_nums(name = "fig25","Difference in pages created before or after hnours (all recipients awarded after 15 Jan 2001 with wikipedia page )")
```
...
* The number of Orders being announced every year is increasing, and the proportion of women receiving honours is increasing, although this is not at parity. The proportion of women and men who have an order and a wikipedia page is a similar ratio overall, but women who have a higher Order level, have a higher chance than average of also having a wikipedia page.

* The announcement of the honours appears to prompt a higher than average number of pages which can be seen on the day and the week of the award. The year an honour is announced also has a higher rate of page creation if the sample is limited to those receieving an honour after the start of wkipedia. The intensity of page creation is also high on the week of the announcement for all honours levels and also when looking at gender.

* The rate of pages being created for people holding an honour appears to be dropping after a peak of creation in 2005-2006. While the number of honours is increasing the nuber of pages is decreasing. The gap between those that have received an honour, and those that have receievd an honour and have a wikipeida page is widening as page creation declines.
